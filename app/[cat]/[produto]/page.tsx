...
export const generateStaticParams: () => Promise<PageParams[]> = async () => {
  ...
  const mercadorias: IItem[] = await (await fetch(API_URL + "Mercadorias").catch(hfe))?.json().catch(hfe) || [];

  return mercadorias.map((merc, i) => ({
...
const fetchMercs: (sku: string) => Promise<fetchTuple> = async (sku: string) => {
  const mercadoria: IItem  = await (await fetch(API_URL + 'Mercadorias?s=' + sku).catch(hfe))?.json().catch(hfe);
  const sugested: IItem[] = await (await fetch(API_URL + 'Mercadorias?rel=&s=' + sku).catch(hfe))?.json().catch(hfe) || [];
